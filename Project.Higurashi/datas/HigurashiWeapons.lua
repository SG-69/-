...
local higurashi_weapon = {}

function higurashi_weapon.give_ammo(pid, hash, type, amount)
    local x = player.get_player_ped(pid)
    amount = amount or 999999
    if type == true then
        weapon.set_ped_ammo_by_type(x, hash, amount)
    elseif type == false then
        weapon.set_ped_ammo(x, hash, amount)
    end
end

function higurashi_weapon.give_weapon_to(pid, hash, now)
    local x = player.get_player_ped(pid)
    if not weapon.has_ped_got_weapon(x, hash) then
        weapon.give_delayed_weapon_to_ped(x, hash, 0, now)
        weapon.set_ped_ammo(x, hash, 9999)
    elseif weapon.has_ped_got_weapon(x, hash) then
        weapon.set_ped_ammo(x, hash, 9999)
    end
end

function higurashi_weapon.remove_weapon_from(pid, hash)
    local x = player.get_player_ped(pid)
    if weapon.has_ped_got_weapon(x, hash) then
        weapon.remove_weapon_from_ped(x, hash)
    end
end

higurashi_weapon.weapons = {
    { "Acid Package", 0xF7F1E25E },
    { "Baseball", 0x23C9F95C },
    { "Briefcase", 0x88C78EB7 },
    { "Briefcase 2", 0x1B79F17 },
    { "BZ Gas", 0xA0973D5E },
    { "Candy Cane", 0x6589186A },
    { "Cougar", 0x8D4BE52 },
    { "Digiscanner", 0xFDBADCED },
    { "EMP Launcher", 0xDB26713A },
    { "Fertilizer Can", 0x184140A1 },
    { "Fire Extinguisher", 0x060EC506 },
    { "Flare", 0x497FACC3 },
    { "Garbadge Bag", 0xE232C28C },
    ...
    { "Metal Detector", 0xDBA2E809 },
    { "Parachute", 0xFBAB5776 },
    { "Precision Rifle", 0x6E7DDDEC },
    { "Railgun", 0x6D544C99 },
    { "Railgun XM13", 0xFEA23564 },
    { "Stungun", 0x3656C8C1 },
    { "Service Carbin", 0xD1D5F52B },
    { "Snowballs", 0x787F0BB },
    { "Unarmed", 0xA2719263 },
    { "WM 29 Pistol", 0x1BC4FDB9 },
}
...
local misc = { { "Fire Extinguisher", 101631238 } }

local all_weapons = { melee_weapons, heavy_weapons, rifles, SMGs, pistols, throwables, shotguns, misc, explosive_weapons }
...
function higurashi_weapon.get_table_of_melee_weapons()
    return get_hashes(melee_weapons)
end

function higurashi_weapon.get_table_of_rifles()
    return get_hashes(rifles)
end

function higurashi_weapon.get_table_of_smgs()
    return get_hashes(SMGs)
end

function higurashi_weapon.get_table_of_explosive_weapons()
    return get_hashes(explosive_weapons)
end

function higurashi_weapon.get_table_of_shotguns()
    return get_hashes(shotguns)
end

function higurashi_weapon.get_table_of_misc_weapons()
    return get_hashes(misc)
end

function higurashi_weapon.get_table_of_throwables()
    return get_hashes(throwables)
end

function higurashi_weapon.get_table_of_pistols()
    return get_hashes(pistols)
end

function higurashi_weapon.get_table_of_heavy_weapons()
    return get_hashes(heavy_weapons)
end

function higurashi_weapon.get_table_of_weapons(rif, smg, sg, pis, xpl, throw, hev, mel, misc)
    local Table = {}
    if rif then
...
function higurashi_weapon.get_all_attachment_info_for_weapon(weapon_hash)
...
function higurashi_weapon.get_weapon_attachment_details(weapon_hash)
    if weapon_attachments[weapon_hash] then
        return weapon_attachments[weapon_hash]
    else
        return {}
...
function higurashi_weapon.get_all_weapon_attachment_details()
    return weapon_attachments
end

function higurashi_weapon.get_maxed_attachments_for_weapon(weapon_hash)
    if weapon_attachments[weapon_hash] then
        local attachments = {}
...
function higurashi_weapon.get_random_attachments_for_weapon(weapon_hash)
    if weapon_attachments[weapon_hash] then
        local attachments = {}
...
function higurashi_weapon.set_ped_weapon_attachments(Ped, random_attachments, weapon_hash)
    weapon.set_ped_weapon_tint_index(Ped, weapon_hash,
        math.random(1, math.max(weapon.get_weapon_tint_count(weapon_hash), 1)))
    local attachments
    if random_attachments then
        attachments = higurashi_weapon.get_random_attachments_for_weapon(weapon_hash)
    else
        attachments = higurashi_weapon.get_maxed_attachments_for_weapon(weapon_hash)
    end
